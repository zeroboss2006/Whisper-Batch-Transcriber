...
def transcribe_file(input_path, language, model, parent_folder, model_choice):
    filename_raw = Path(input_path).stem
    filename_stem = re.sub(r'[\\/:*?"<>|]', "_", filename_raw) + f"({model_choice})"
...
def get_model_choice(model_input):
    model_map = {"1": "base", "2": "medium", "3": "large-v2",
                 "base": "base", "medium": "medium", "large-v2": "large-v2"}
    return model_map.get(model_input.lower(), "base")
...
def interactive_mode():
    ...
    print("\n請選擇模型：\n1. base\n2. medium\n3. large-v2")
    model_choice = get_model_choice(input("輸入數字 [1-3]，預設為 1：").strip())
    ...
    if input_path_obj.is_file():
        transcribe_file(str(input_path_obj), language, model, input_path_obj.parent, model_choice)
    elif input_path_obj.is_dir():
        files = [f for f in input_path_obj.glob("*") if f.suffix.lower() in exts]
        existing_zips = [f"{re.sub(r'[\\/:*?\"<>|]', '_', f.stem)}({model_choice}).zip" for f in files if (input_path_obj / f"{re.sub(r'[\\/:*?\"<>|]', '_', f.stem)}({model_choice}).zip").exists()]
        if existing_zips:
        ...
        for media_file in tqdm(files, desc="批次處理中", ncols=80):
            try:
                transcribe_file(str(media_file), language, model, input_path_obj, model_choice)
...
def arg_mode(args):
    if args.input_file:
        ...
        input_paths = [f for f in folder.glob("*") if f.suffix.lower() in exts]
    else:
        print("請提供 --input-file 或 --input-folder")
        exit()

    language = "English" if args.language == "en" else "Chinese"
    model_choice = get_model_choice(args.model)
    ...
    if args.input_folder:
        folder = Path(args.input_folder)
        existing_zips = [f"{re.sub(r'[\\/:*?\"<>|]', '_', f.stem)}({model_choice}).zip" for f in input_paths if (folder / f"{re.sub(r'[\\/:*?\"<>|]', '_', f.stem)}({model_choice}).zip").exists()]
    ...
    bar = tqdm(input_paths, desc="批次處理中", ncols=80) if len(input_paths) > 1 else input_paths
    for p in bar:
        try:
            transcribe_file(str(p), language, model, p.parent, model_choice)
        except Exception as e:
            print(f"❌ 轉換失敗：{p.name} ({e})")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Whisper 語音轉檔/批次處理工具 (繁體強制轉換)")
    parser.add_argument('--input-file', help='單一檔案路徑')
    parser.add_argument('--input-folder', help='資料夾路徑（批次）')
    parser.add_argument('--language', choices=['zh', 'en'], help='語言 zh=中文(預設), en=英文', default='zh')
    parser.add_argument('--model', choices=['base', 'medium', 'large-v2'], default='base', help='Whisper模型')
    parser.add_argument('--device', choices=['auto', 'cpu', 'gpu'], default='auto', help='運算裝置')
    parser.add_argument('--no-prompt', action='store_true', help='跳過覆蓋確認')
    args, unknown = parser.parse_known_args()

    # 沒有參數時啟動互動式
    if not any([args.input_file, args.input_folder]):
        interactive_mode()
    else:
        arg_mode(args)
